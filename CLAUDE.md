...
**A bump alone never reaches this machine's sessions** (dev-orchestration iter 11): the install record in `~/.claude/plugins/installed_plugins.json` stays pinned to the old version's cache dir, and a directory-source marketplace never auto-updates it — qino sat silently at 3.0.0 for four months this way. Every version bump ends with `claude plugin update --scope project <plugin>@qino-plugins` + a session restart (`/release` step 10). The workspace SessionStart advisory hook warns on source-vs-installed drift as a backstop.
