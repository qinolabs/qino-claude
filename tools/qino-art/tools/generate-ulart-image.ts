...
async function loadEnv(): Promise<void> {
  // Check local repo first (ecologically contained)
  const localEnv = resolve(process.cwd(), '.claude/.env');
  const rootEnv = resolve(process.cwd(), '.env');
  const globalEnv = resolve(process.env.HOME!, '.claude/.env');
  
  const envPath = await (async () => {
    try { if ((await readFile(localEnv)).length > 0) return localEnv; } catch {}
    try { if ((await readFile(rootEnv)).length > 0) return rootEnv; } catch {}
    return globalEnv;
  })();
  try {
